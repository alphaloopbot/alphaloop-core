...
class AlphaLoopLogger:
    ...
    def _setup_handlers(self) -> None:
        ...
        if self.config.telegram_config:
            telegram_handler = TelegramHandler(self.config.telegram_config, self.config.hostname)
    ...
    async def critical(
        ...
        record = self._create_record(LogLevel.CRITICAL, message, info, telegram, **extra)
        await self._emit_to_handlers(record)
    ...
    def log_sync(self, level: str, message: str) -> None:
        ...
        This method allows using the async logger in synchronous contexts
        by automatically handling the event loop management.
        ...
        try:
            ...
            if loop.is_running():
                # If we're already in an event loop, schedule the log
                asyncio.create_task(getattr(self, level)(message))
            else:
                # If no event loop is running, run the log
                loop.run_until_complete(getattr(self, level)(message))
        except RuntimeError:
