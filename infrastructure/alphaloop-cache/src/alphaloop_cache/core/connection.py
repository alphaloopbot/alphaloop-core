...
class CacheConfig:
    ...
    def from_env(cls, prefix: str = "CACHE_") -> "CacheConfig":
        ...
        return cls(
            ...
            retry_on_timeout=os.getenv(f"{prefix}RETRY_ON_TIMEOUT", "true").lower()
            == "true",
            socket_connect_timeout=int(
                os.getenv(f"{prefix}SOCKET_CONNECT_TIMEOUT", "5")
            ),
            socket_timeout=int(os.getenv(f"{prefix}SOCKET_TIMEOUT", "5")),
            decode_responses=os.getenv(f"{prefix}DECODE_RESPONSES", "true").lower()
            == "true",
    ...
    async def key_exists(self, key: str) -> bool:
        ...
        try:
            ...
            raise CacheOperationError(
                f"Failed to check key existence {key}: {e}"
            ) from e
